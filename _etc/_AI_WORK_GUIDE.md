...
- 커밋 메시지는 다음과 같이 구성되어있다.
  ...
  2. 커밋 내용 (타이틀과 한 줄 띄고 작성)
    ...
    - **간결성 원칙**: 커밋 메시지는 변경 사항의 요약만 포함한다.
      - 구체적인 파일명이나 함수명을 나열하지 않는다.
      - 상세한 구현 내용(Redux slice, hook, 소켓 리스너 등)을 나열하지 않는다.
      - 상세한 변경 내역은 `git diff`로 확인 가능하므로 커밋 메시지에는 일반적인 설명만 작성한다.
      - 예시:
        - ❌ 나쁜 예: `ChatListObject: handleWheel → onWheel, ChatInputGroup: handleWheel → onWheel, BlogPage: handleImageClick → onClickImage...`
        - ❌ 나쁜 예: `Redux slice에 새 채팅방 추가 액션 구현, Redux hook에 액션 추가, 소켓 이벤트 리스너에 핸들러 추가`
        - ✅ 좋은 예: `handle로 시작하는 함수들을 onClick, onSubmit, onChange 등으로 변경`
        - ✅ 좋은 예: `새 채팅방 생성 시 실시간 알림 기능 추가`
...
- 커밋 메시지 간결성 원칙 추가: 2025-12-07
- 커밋 메시지 간결성 원칙 보완 (상세 구현 내용 나열 금지): 2025-12-14
