...
import {AddDirectoryObject, AddFileObject, FileRowPObject} from '../'
...
type DirectoryRowPObjectProps = DivCommonProps & {
  dirIdx: number
  dirOId: string
  parentDirOId: string
}

export const DirectoryRowPObject: FC<DirectoryRowPObjectProps> = ({dirIdx, dirOId, parentDirOId, className, ...props}) => {
  ...
  return (
    <div
      className={`DirectoryRow_PObject ${className || ''}`}
      ...
      {isOpen && (
        <div onDragOver={e => e.preventDefault()} style={{marginLeft: '8px'}}>
          ...
          {directory.subDirOIdsArr.map((subDirOId, subDirIdx) => (
            <DirectoryRowPObject key={subDirIdx} dirIdx={subDirIdx} dirOId={subDirOId} parentDirOId={dirOId} />
          ))}
          ...
          {directory.fileOIdsArr.map((fileOId, fileIdx) => (
            <FileRowPObject key={fileIdx} parentDirOId={dirOId} fileIdx={fileIdx} fileOId={fileOId} />
          ))}
