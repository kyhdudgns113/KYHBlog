...
import {AUTH_USER} from '@commons/secret'
...
export class CheckAuth extends GKDTestBase {
  ...
  protected async execTest(db: mysql.Pool, logLevel: number) {
    try {
      await this.memberFail(this._1_User0_try_to_User1_ChatRoom.bind(this), db, logLevel)
      // ::
    } catch (errObj) {
      // ::
      throw errObj
  ...
  private async _1_User0_try_to_User1_ChatRoom(db: mysql.Pool, logLevel: number) {
    try {
      ...
      await this.portService.loadChatRoomArr(jwtPayload, userOId_user_1)
      // ::
    } catch (errObj) {
      // ::
