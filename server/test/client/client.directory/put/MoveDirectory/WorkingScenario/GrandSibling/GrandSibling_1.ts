...
export async function _1_FrontToGrandFront(db: mysql.Pool, logLevel: number) {
  /**
   * 2-1. 0번째 디렉토리의 0번째 자식을 루트의 0번째 자식으로 만든다.
   *
   * 점검사항
  ...
  try {
    ...
    const moveDirOId = this.dirOIds['dir0_0']
...
export async function _2_FrontToSiblingFront(db: mysql.Pool, logLevel: number) {
  /**
   * 2-2. 루트의 0번째 자식을 원래 0번째 디렉토리의 0번째 자식으로 만든다
   *
   * 점검사항
   *   1. extraDirs 길이가 15인가?
   *   2. extraDirs 의 0번째 dirOId 가 기존 부모 디렉토리인가?
  ...
  try {
    ...
    if (extraDirs.dirOIdsArr[4] !== newParentDirOId) {
      throw `5. 배열의 4번째 원소가 ${newParentDirOId} 이 아닌 ${extraDirs.dirOIdsArr[11]} 이다.`
    }
    // 6. extraDirs 의 12번째 부터 14번째 dirOId 가 입력값과 일치한가?
    for (let i = 5; i < 15; i++) {
...
export async function _3_FrontToGrandMiddle(db: mysql.Pool, logLevel: number) {
  /**
   * 2-3. 0번째 디렉토리의 0번째 자식을 루트의 가운데로 이동한다.
   *
   * 점검사항
   *   1. extraDirs 길이가 15인가?
   *   2. extraDirs 의 0번째 dirOId 가 기존 부모 디렉토리인가?
...
export async function _4_MiddleToSiblingFront(db: mysql.Pool, logLevel: number) {
  /**
   * 2-4. 루트의 가운데 자식을 원래 0번째 디렉토리의 0번째 자식으로 만든다
   *
   * 점검사항
   *   1. extraDirs 길이가 15인가?
   *   2. extraDirs 의 0번째 dirOId 가 기존 부모 디렉토리인가?
...
export async function _5_FrontToGrandBack(db: mysql.Pool, logLevel: number) {
  /**
   * 2-5. 0번째 디렉토리의 0번째 자식을 루트의 맨 뒤로 이동한다.
   *
   * 점검사항
   *   1. extraDirs 길이가 15인가?
   *   2. extraDirs 의 0번째 dirOId 가 기존 부모 디렉토리인가?
...
export async function _6_BackToSiblingFront(db: mysql.Pool, logLevel: number) {
  /**
   * 2-6. 루트의 맨 뒤 자식을 원래 0번째 디렉토리의 0번째 자식으로 만든다
   *
   * 점검사항
   *   1. extraDirs 길이가 15인가?
   *   2. extraDirs 의 0번째 dirOId 가 기존 부모 디렉토리인가?
