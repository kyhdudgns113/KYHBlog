...
export class EditCommentFunction extends GKDTestBase {
  ...
  protected async execTest(db: mysql.Pool, logLevel: number) {
    try {
      ...
      this.addFinalLog(`[EditCommentFunction] 테스트 작성 안됨`, consoleColors.FgYellow)
      // ::
    } catch (errObj) {
      // ::
      throw errObj
...
if (require.main === module) {
  ...
  const testModule = new EditCommentFunction(DEFAULT_REQUIRED_LOG_LEVEL) // __Test 대신에 모듈 이름 넣는다.
