...
export class DeleteReplyFunction extends GKDTestBase {
  ...
  protected async execTest(db: mysql.Pool, logLevel: number) {
    try {
      ...
      this.addFinalLog(`[DeleteReplyFunction] 테스트 작성 안됨`, consoleColors.FgYellow)
      // ::
    } catch (errObj) {
      // ::
      throw errObj
...
if (require.main === module) {
  ...
  const testModule = new DeleteReplyFunction(DEFAULT_REQUIRED_LOG_LEVEL) // __Test 대신에 모듈 이름 넣는다.
